...
class VectorStore:
    ...
    def all_documents(self) -> list[dict]:
        """[{"id", "text", "metadata"}, ...] for the whole collection.

        Used to build the lexical (BM25) index over the same entities the
        vector index holds -- there is exactly one knowledge base.
        """
        raw = self._collection.get(include=["documents", "metadatas"])
        if not raw["ids"]:
            return []
        return [
            {"id": doc_id, "text": doc or "", "metadata": meta or {}}
